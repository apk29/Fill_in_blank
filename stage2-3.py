...
def match_items(word,blanks):
  for items in blanks: #blanks = ["__1__", "__2__", "__3__","__4__"]
    if items in word: #items in the split, split = paragraphs list = easy_paragraph, medium paragraph, or hard_paragraph, depends on your level choice
      return items
  return None
...
def fill_blank_in_paragraph(paragraphs,player_guess,replaced,index):
    ...
    for word in split:
        replacement = match_items(word,blanks)
        if replacement != None:            
...
def quiz_answer(answers,paragraphs):
    ...
    while index < 4: 
        ...
        if player_guess == answers[index]:
            print "Correct!"
            replaced = fill_blank_in_paragraph(paragraphs,player_guess,replaced, index)           
            index = index + 1             
        else:

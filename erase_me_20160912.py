...
def get_setup(level):
    ...
    if level == "1":
        ...
        return easy_paragraph, easy_answers #[perry] why is the easy_answers need to be returned here?
...
def word_in_blanks(word, blanks): #[Perry] I understand the blanks is the list above, what does word repesent? I think it is either the (easy, medium, & hard )answer or paragraph above?
...
def collect_answers(level, paragraph, answers): #[Perry] I can see here that level can be referenced to the level in get_set(level) above, does the paragraph & answer represent the easy, medium, hard paragraph & answers list above?
